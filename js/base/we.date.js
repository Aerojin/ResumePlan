...
(function (WE) {
    ...
    */
    format: function (text, date) {
        ...
        if (/(y+)/.test(text)) text = text.replace(RegExp.$1, (date.getFullYear() + "").substr(4 - RegExp.$1.length));
        for (var k in o)
            if (new RegExp("(" + k + ")").test(text)) text = text.replace(RegExp.$1, (RegExp.$1.length == 1) ? (o[k]) : (("00" + o[k]).substr(("" + o[k]).length)));
        return text;
    },
    /**
    ...
    */
    getServerTime: function () {
        var d = new Date();
        var diffTime = (top.WE && top.WE._ClientDiffTime_) || WE._ClientDiffTime_;
        if (diffTime) {
            //如果WE.HttpClient已通过ajax请求获取过服务器与客户端的时间差
    ...
    */
    ...
    /**
    * 将严格合法的日期字符串解析为日期对象，如果不合法，返回null
    * 默认年份限制为1970年~2100年之
    * 包含以下列举格式：
    * "yyyy-MM-dd", "yyyy/MM/dd", "yyyy/MM/dd"
    * "yyyy-M-d", "yyyy/M/d", "yyyyMMdd"
    */
    parseValidDateStr: function (str, startYear, endYear) {
        var year, month, day;
        var matches;
        if (startYear === undefined) startYear = 1970;
        if (endYear === undefined) endYear = 2100;

        if (matches = str.match(/^(\d{4})([\.\/-]?)(\d{1,2})\2(\d{1,2})$/)) {
            year = parseInt(matches[1], 10);
            month = parseInt(matches[3], 10);
            day = parseInt(matches[4], 10);
            if (day === 0 || month === 0 || day > 31 || month > 12 || year < startYear || year > endYear) {
                return null;
            }
            if (month === 2 && day > 28 + (year % 400 == 0 || (year % 4 == 0 && year % 100 != 0))) {
                return null;
            }
            if ("46911".indexOf(month) >= 0 && day === 30) {
                return null;
            }
            if (matches[2] == "" && (matches[3] + matches[4]).length != 4) {	// 无分隔符，必须是yyyyMMdd格式
                return null;
            }
...
Date.prototype.format = function (template) {
    return WE.Date.format(template, this);
}
